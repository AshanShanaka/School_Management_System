...
export default function ParentOLPredictionPage() {
  ...
  useEffect(() => {
    if (selectedChild) {
      setLoading(true);
      fetch(`/api/predictions/parent/${selectedChild}`)
        .then(r => r.json())
        .then(res => { 
    ...
    <div className="p-6 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
      ...
      <div className="mb-6 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 rounded-2xl p-8 shadow-xl text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold mb-2">Child O/L Prediction Dashboard</h1>
          ...
          {children.length > 0 && (
            <select 
              ...
              className="bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-lg border border-white/30"
            >
              {children.map(child => (
